...
def find_attendee_section(rows: list[list[str]]) -> int:
    ...
    for index, row in enumerate(rows):
        if any(
            normalise_column_name(value) == normalise_column_name(ATTENDEE_MARKER)
            for value in row
        ):
    ...
    for index, row in enumerate(rows):
        resolved_roles = resolve_attendee_columns(value.strip() for value in row)
        if COMPACT_ATTENDEE_HEADER_ROLES.issubset(resolved_roles):
            return index

    raise ValueError(
        f"Could not find the '{ATTENDEE_MARKER}' section or an attendee header"
    )


def extract_report_value(rows: list[list[str]], *field_names: str) -> str:
    """Extract a named value from the report preamble."""
    normalised_field_names = {
        normalise_column_name(field_name) for field_name in field_names
    }
    for index, row in enumerate(rows[:-1]):
        labels = [normalise_column_name(value) for value in row]
        for normalised_field_name in normalised_field_names:
    ...
    expected_fields = " or ".join(repr(field_name) for field_name in field_names)
    raise ValueError(f"Could not find {expected_fields} in the report preamble")
...
def prepare_attendee_dataframe(input_path: Path) -> pd.DataFrame:
    ...
    dataframe["WebinarID"] = extract_report_value(rows, "Webinar ID", "ID")
    connection_columns = {
        columns[role]
...
def clear_output_workbooks(output_folder: Path = OUTPUT_FOLDER) -> int:
    """Remove workbooks generated by an earlier pipeline run."""
    removed_count = 0
    for workbook_path in output_folder.glob("*.xlsx"):
...
def main() -> None:
    ...
    removed_count = clear_output_workbooks(args.output_folder)
    print(
        f"Cleared {removed_count} existing workbook(s) from "
